-- =================================================== --
-- PFD for Cross-Layout Copters / Made by;
-- =================================================== --
--                     ,--,                            
--                  ,---.'|                            
--     ,---,        |   | :        ,---,.   .--.--.    
...
local function wizard()
    ...
    local savedIsCross = saved
        and saved.F and saved.R and saved.B and saved.L
    if saved and not savedIsCross then
        print("-- Old X-quad config detected (FL/FR/BL/BR) --")
        print("Cross layout uses F/R/B/L.  Re-running wizard.")
        print("")
        saved = nil
    end
    if saved then
        ...
        linePair("F",   shortPeri(saved.F),   "R", shortPeri(saved.R))
        linePair("B",   shortPeri(saved.B),   "L", shortPeri(saved.L))
        print(" Redstone")
    ...
    print("attitude rotors arranged in a CROSS (+) -- one motor on")
    print("each body axis.  F/B trim pitch; L/R trim roll.")
    print("Convention: F+ rpm pitches NOSE UP.")
    print("")
    ...
    local f   = ask("Attitude F (front)", "RSC 1")
    local r   = ask("Attitude R (right)", "RSC 2")
    local b   = ask("Attitude B (back)",  "RSC 3")
    local l   = ask("Attitude L (left)",  "RSC 4")
    print("")
    ...
    for lbl, name in pairs({GLM=glm, F=f, R=r, B=b, L=l,
...
local G_stab_flux = Graph.new("STAB - Motor Flux", {
    { name = "F",  color = { 90, 220, 130 } },
    { name = "R",  color = { 235, 220, 90 } },
    { name = "B",  color = { 240, 160, 70 } },
    { name = "L",  color = { 90, 200, 235 } },
})
...
local function drawSidePanel(d, gainsKind)
    ...
    local function blockStab()
        ...
        motorBar("F", d.f, { 90,220,130 })
        motorBar("R", d.r, { 235,220,90 })
        motorBar("B", d.b, { 240,160,70 })
        motorBar("L", d.l, { 90,200,235 })
        div()
    end
...
local function ingestFrame(d)
    G_stab_flux:push({ F=d.f, R=d.r, B=d.b, L=d.l })
...
local function setMotors(coll, pitchOut, rollOut)
    ...
    local f   = roundI(clamp(ATT_BASE + pitchOut, MOTOR_MIN, MOTOR_MAX))
    local b   = roundI(clamp(ATT_BASE - pitchOut, MOTOR_MIN, MOTOR_MAX))
    local r   = roundI(clamp(ATT_BASE + rollOut,  MOTOR_MIN, MOTOR_MAX))
    local l   = roundI(clamp(ATT_BASE - rollOut,  MOTOR_MIN, MOTOR_MAX))
    M.GLM.setTargetSpeed(glm)
    M.F.setTargetSpeed(f); M.B.setTargetSpeed(b)
    M.R.setTargetSpeed(r); M.L.setTargetSpeed(l)
    return glm, f, r, b, l
end
...
local function makeControlStep()
    ...
    return function()
        ...
        local glm, f, r, b, l = setMotors(coll, pitchOut, rollOut)
