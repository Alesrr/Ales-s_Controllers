...
local COL_FL = colors.lime
local COL_FR = colors.yellow
local COL_BL = colors.cyan
local COL_BR = colors.orange
...
local function wizard()
    ...
    local fl  = ask("Motor FL", "RSC 1")
    local fr  = ask("Motor FR", "RSC 2")
    local bl  = ask("Motor BL", "RSC 3")
    local br  = ask("Motor BR", "RSC 4")
...
local function makeControlStep(mon, mW, mH)
    ...
    return function()
        ...
        -- Standard X-quad mix:
        -- pitchOut > 0 => need nose-up correction => raise front, drop back
        -- rollOut  > 0 => need right-up correction => raise right, drop left
